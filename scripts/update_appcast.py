...
def update_appcast(message):
    ...
    release_file = Path(f"release/bob-plugin-mimo-2-tts.bobplugin")
    ...
    version_info = {
        ...
        "desc": message,
        "sha256": file_hash,
        "url": f"https://github.com/ipfans/bob-plugin-mimo-2-tts/releases/download/v{version}/bob-plugin-mimo-2-tts_v{version}.bobplugin",
        "minBobVersion": "0.5.0",
    }
    ...
    if appcast_file.is_file():
        ...
        appcast = dict(identifier="com.acgso.bob-plugin-mimo-2-tts", versions=[])
